...
{
 { "asound",    do_mpasound,     "will soon" },
 { "gecho",     do_mpgecho,      "will soon" },
 { "zecho",     do_mpzecho,      "will soon" },
 { "kill",      do_mpkill,       "will soon" },
 { "assist",    do_mpassist,     "will soon" },
 { "junk",      do_mpjunk,       "will soon" },
 { "echo",      do_mpecho,       "will soon" },
 { "echoaround",do_mpechoaround, "will soon" },
 { "echoat",    do_mpechoat,     "will soon" },
 { "mload",     do_mpmload,      "will soon" },
 { "oload",     do_mpoload,      "will soon" },
 { "purge",     do_mppurge,      "will soon" },
 { "goto",      do_mpgoto,       "will soon" },
 { "at",        do_mpat,         "will soon" },
 { "transfer",  do_mptransfer,   "will soon" },
 { "gtransfer", do_mpgtransfer,  "will soon" },
 { "otransfer", do_mpotransfer,  "will soon" },
 { "force",     do_mpforce,      "will soon" },
 { "gforce",    do_mpgforce,     "will soon" },
 { "vforce",    do_mpvforce,     "will soon" },
 { "cast",      do_mpcast,       "will soon" },
 { "damage",    do_mpdamage,     "will soon" },
 { "remember",  do_mpremember,   "will soon" },
 { "forget",    do_mpforget,     "will soon" },
 { "delay",     do_mpdelay,      "will soon" },
 { "cancel",    do_mpcancel,     "will soon" },
 { "call",      do_mpcall,       "will soon" },
 { "flee",      do_mpflee,       "will soon" },
 { "remove",    do_mpremove,     "will soon" },
 { "mpslay",    do_mpslay,       "will soon" },
 { "",          0,               "will soon" }
};
...
{
  // Security check!
  if ( ch->desc != NULL && get_trust(ch) < MAX_LEVEL ) return;
...
void mob_interpret( CHAR_DATA *ch, const char *argument )
{
  ...
  do_printf( buf, "Mob_interpret: invalid cmd from mob %u: '%s'",
...
void do_mpstat( CHAR_DATA *ch, const char *argument )
{
  ...
  {
      stc( "No such creature.\n\r", ch );
      return;
  }

  if ( !IS_NPC( victim ) )
  {
      stc( "That is not a mobile.\n\r", ch);
      return;
  }

  if ( ( victim = get_char_world( ch, arg ) ) == NULL )
  {
      stc( "No such creature visible.\n\r", ch );
      return;
  }

  do_printf( arg, "Mobile #%-6d [%s]\n\r",
      victim->pIndexData->vnum, get_char_desc(victim,'1') );
  stc( arg, ch );
  ...
  {
      do_printf( arg, "[%2d] Trigger [%-8s] Program [%4d] Phrase [%s]\n\r",
            ++i,
...
void do_mpgecho( CHAR_DATA *ch, const char *argument )
{
  ...
  if ( argument[0] == '\0' )
  {
      bug( "MpGEcho: missing argument from vnum %u",
...
void do_mpzecho( CHAR_DATA *ch, const char *argument )
{
  ...
  if ( argument[0] == '\0' )
  {
      bug( "MpZEcho: missing argument from vnum %u",
          IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
      return;
  }
...
void do_mpkill( CHAR_DATA *ch, const char *argument )
{
  ...
  {
      bug( "MpKill - Charmed mob attacking master from vnum %u.",
          IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
      return;
  }
...
void do_mpechoat( CHAR_DATA *ch, const char *argument )
{
  ...
  if ( arg[0] == '\0' || argument[0] == '\0' ) return;
...
void do_mpmload( CHAR_DATA *ch, const char *argument )
{
  ...
  if ( ch->in_room == NULL || arg[0] == '\0' || !is_number(arg) ) return;

  vnum = atoi64(arg);
  if ( ( pMobIndex = get_mob_index( vnum ) ) == NULL )
  {
      do_printf( arg, "Mpmload: bad mob index (%u) from mob %u",
...
void do_mpoload( CHAR_DATA *ch, const char *argument )
{
   ...
   */
  if ( arg3[0] == 'R' || arg3[0] == 'r' ) fToroom = TRUE;
  else if ( arg3[0] == 'W' || arg3[0] == 'w' ) fWear = TRUE;
  ...
  if ((obj = create_object( pObjIndex, level )))
  {
   ...
   {
    obj_to_char( obj, ch );
    if ( fWear ) wear_obj( ch, obj, TRUE, TRUE );
   }
   else
   {
...
void do_mppurge( CHAR_DATA *ch, const char *argument )
{
  ...
  if ( ( victim = get_char_room( ch, arg ) ) == NULL )
  {
      ...
      {
          bug( "Mppurge - Bad argument from vnum %u.",
              IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
      }
      return;
  }
...
void do_mpat( CHAR_DATA *ch, const char *argument )
{
  ...
  if ( ( location = find_location( ch, arg ) ) == NULL )
  {
      bug( "Mpat - No such location from vnum %u.",
              IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
      return;
  }
...
void do_mptransfer( CHAR_DATA *ch, const char *argument )
{
  ...
  if ( !str_cmp( arg1, "all" ) )
  {
      ...
      for ( victim = ch->in_room->people; victim != NULL; victim = victim_next )
      {
          ...
          if ( !IS_NPC(victim) )
          {
              do_printf( buf, "%s %s", victim->name, arg2 );
  ...
  else
  {
      ...
      {
          bug( "Mptransfer - No such location from vnum %u.",
              IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
          return;
      }
  ...
  if ((victim=get_char_room(ch,arg1))==NULL && (victim=get_char_world(ch,arg1))==NULL)
...
void do_mpforce( CHAR_DATA *ch, const char *argument )
{
  ...
  if ( arg[0] == '\0' || argument[0] == '\0' )
  {
    bug( "Mpforce - Bad syntax from vnum %u.", IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
    return;
  }
  ...
  else
  {
    ...
    if (IS_SET(cmd_table[cmd].flag,NOFORCE)) return;
    if (IS_ELDER(victim)) return;
...
void do_mpflee( CHAR_DATA *ch, const char *argument )
{
  ...
  if ( (was_in = ch->in_room) == NULL ) return;
...
void do_mpotransfer( CHAR_DATA *ch, const char *argument )
{
  ...
  if ( (obj = get_obj_here( ch, arg )) == NULL ) return;
  if ( obj->carried_by == NULL ) obj_from_room( obj );
  else
  {
...
void do_mplist( CHAR_DATA *ch, const char *argument )
{
  int count;
  MPROG_CODE *mprg;
  char buf[MAX_STRING_LENGTH];
...
void do_mpremove( CHAR_DATA *ch, const char *argument )
{
  ...
  argument = one_argument( argument, arg );
  if (!(victim=get_char_room(ch,arg)) ) return;

  one_argument( argument, arg );
  if ( !str_cmp( arg, "all" ) ) fAll = TRUE;
  else if ( !is_number( arg ) )
  {
    bug ("MpRemove: Invalid object from vnum %u.",
      IS_NPC(ch)?ch->pIndexData->vnum : 0 );
    return;
  }
  else vnum = atoi64( arg );
...
/* Slays mobile or target in argument.
 * Slays the mobile itself must be at the end of mobprogram!!!!!!! Saboteur.
 *
 * syntax mob slay {target}
 */
void do_mpslay( CHAR_DATA *ch, const char *argument )
{
  ...
  if (found) extract_char(victim,TRUE);
}
