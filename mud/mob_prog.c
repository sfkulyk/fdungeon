...
// if-check keywords:
const struct keywords_type fn_keyword[] =
{
  {"rand",      "if rand 30             - if rnd(100) < 30"        },
  {"mobhere",   "if mobhere [fido|3021] - 'fido' or mob 3021 here" },
  {"objhere",   "if objhere [cup|112]   - 'cup' or obj 112 here"   },
  {"mobexists", "if mobexists fido      - is 'fido' somewhere"          },
  {"objexists", "if objexists sword     - is 'sword' somewhere"         },

  {"people",    "if people > 4   - does in room > 4 people"},
  {"players",   "if players > 1  - does in room > 1 pcs"   },
  {"mobs",      "if mobs > 2     - does in room > 2 mobiles" },
  {"clones",    "if clones > 3   - does in room > 3 mobs of same vnum" },
  ...
  {"carries",   "if carries $n bag|12 - does $n have a 'bag'|obj 12"  },
  {"wears",     "if wears $n mace|12  - is $n wearing a 'mace'|obj 12"},
  {"has",       "if has $n weapon     - does $n have obj_weapon"      },
  {"uses",      "if uses $n armor     - is $n wearing obj_armor"      },
  {"name",      "if name $n puff      - is $n's name 'puff'"          },
  {"pos",       "if pos $n standing   - is $n standing"               },
  {"clan",      "if clan $n 'loner'   - does $n clan is 'loner'"      },
  ...
  {"money",     "if money $n"                                       },
  {"objval0",   "if objval0 $o > 1000 - object value[0]" },
  {"objval1",   "if objval1 $o > 1000 - object value[1]" },
  {"objval2",   "if objval2 $o > 1000 - object value[2]" },
  {"objval3",   "if objval3 $o > 1000 - object value[3]" },
  {"objval4",   "if objval4 $o > 1000 - object value[4]" },
  {"grpsize",   "if grpsize $n > 6    - group size check"},
  {"\n"         "Table terminator" }
};

const struct keywords_type fn_evals[] =
{
  { "==", "ðàâíî"        },
  { ">=", "áîëüøå ðàâíî" },
  { "<=", "ìåíüøå ðàâíî" },
...
int keyword_lookup( const struct keywords_type *table, char *keyword )
{
  register int i;
  for( i = 0; table[i].name[0] != '\n'; i++ )
    if( !str_cmp( table[i].name, keyword ) ) return( i );
  return -1;
}

// Perform numeric evaluation. Called by cmd_eval()
int64 num_eval( int64 lval, int oper, int64 rval )
{
  switch( oper )
  {
    case EVAL_EQ:
...
CHAR_DATA *get_random_char( CHAR_DATA *mob )
{
  ...
  {
    if ( mob != vch && !IS_NPC( vch ) && can_see( mob, vch,CHECK_LVL )
...
int get_order( CHAR_DATA *ch )
{
  ...
  {
    if ( vch == ch ) return i;
    if ( IS_NPC(vch) && vch->pIndexData->vnum == ch->pIndexData->vnum ) i++;
  }
  return 0;
}
...
bool has_item( CHAR_DATA *ch, int64 vnum, int item_type, bool fWear )
{
  OBJ_DATA *obj;
  for ( obj = ch->carrying; obj; obj = obj->next_content )
    if ( ( vnum < 0 || obj->pIndexData->vnum == vnum )
     &&   ( item_type < 0 || obj->pIndexData->item_type == item_type )
     &&   ( !fWear || obj->wear_loc != WEAR_NONE ) ) return TRUE;
  return FALSE;
}

// Check if there's a mob with given vnum in the room
bool get_mob_vnum_room( CHAR_DATA *ch, int64 vnum )
{
  CHAR_DATA *mob;
...
bool get_obj_vnum_room( CHAR_DATA *ch, int64 vnum )
{
  OBJ_DATA *obj;
...
int64 cmd_eval( int64 vnum, const char *line, int check,CHAR_DATA *mob,
   CHAR_DATA *ch, const void *arg1, const void *arg2, CHAR_DATA *rch )
{
  ...
  if ( buf[0] == '\0' || mob == NULL ) return FALSE;

  // If this mobile has no target, let's assume our victim is the one
  if ( mob->mprog_target == NULL ) mob->mprog_target = ch;
  ...
  if ( rval >= 0 )
  {
    ...
    {
      do_printf( buf, "Cmd_eval: prog %u syntax error(2) '%s'",vnum, original );
      bug( buf, 0 );
      return FALSE;
    }
  ...
  if ( buf[0] != '$' || buf[1] == '\0' )
  {
    do_printf( buf, "Cmd_eval: prog %u syntax error(3) '%s'",vnum, original );
    bug( buf, 0 );
    return FALSE;
  }
  else code = buf[1];
  switch( code )
  {
    ...
    default:
        do_printf( buf, "Cmd_eval: prog %u syntax error(4) '%s'",vnum, original );
        bug( buf, 0 );
        return FALSE;
  }

  // From now on, we need an actor, so if none was found, bail out
  if ( lval_char == NULL && lval_obj == NULL ) return FALSE;
  ...
  switch( check )
  {
      ...
      case CHK_ISVISIBLE:
          switch( code )
          {
              ...
              case 'r':
              case 'q':
                  return( lval_char != NULL && can_see( mob, lval_char,CHECK_LVL ) );
              case 'o':
              case 'p':
  ...
  if ( (oper = keyword_lookup( fn_evals, buf )) < 0 )
  {
      do_printf( buf, "Cmd_eval: prog %u syntax error(5): '%s'",
              vnum, original );
      bug( buf, 0 );
      return FALSE;
  }
        ...
        const void *arg1, const void *arg2, CHAR_DATA *rch )
{
    ...
    const char *someone = "­¥ªâ®";
    const char *something = "­¥çâ®";
    const char *someones = "ªâ®-â®";
    ...
    while ( *str != '\0' )
    {
        ...
        switch ( *str )
        {
            ...
            case 'q':
                i = someone;
                if ( mob->mprog_target != NULL && can_see( mob, mob->mprog_target,CHECK_LVL ) )
                {
        ...
        CHAR_DATA *mob, CHAR_DATA *ch, const void *arg1, const void *arg2 )
{
    ...
    int64 cond[MAX_NESTED_LEVEL];  /* Boolean value based on the last if-check */

    int64 mvnum = mob->pIndexData->vnum;

    if( ++call_level > MAX_CALL_LEVEL )
    {
        bug( "MOBprogs: MAX_CALL_LEVEL exceeded, vnum %u", mob->pIndexData->vnum );
        return;
    }
    ...
    while ( *code )
    {
        ...
        *b = *c = *d = '\0';
        ...
        if ( !str_cmp( control, "if" ) )
        {
            ...
            {
                do_printf( buf, "Mobprog: misplaced if statement, mob %u prog %u",
                        mvnum, pvnum );
                bug( buf, 0 );
                return;
            }
            ...
            {
                do_printf( buf, "Mobprog: Max nested level exceeded, mob %u prog %u",
                        mvnum, pvnum );
                bug( buf, 0 );
                return;
            }
            ...
            {
                do_printf( buf, "Mobprog: invalid if_check (if), mob %u prog %u",
                        mvnum, pvnum );
                bug( buf, 0 );
                return;
            }
        ...
        else if ( !str_cmp( control, "or" ) )
        {
            ...
            {
                do_printf( buf, "Mobprog: or without if, mob %u prog %u",
                        mvnum, pvnum );
                bug( buf, 0 );
                return;
            }
            ...
            {
                do_printf( buf, "Mobprog: invalid if_check (or), mob %u prog %u",
                        mvnum, pvnum );
                bug( buf, 0 );
                return;
            }
        ...
        else if ( !str_cmp( control, "and" ) )
        {
            if ( !level || state[level-1] != BEGIN_BLOCK )
            {
                do_printf( buf, "Mobprog: and without if, mob %u prog %u",
                        mvnum, pvnum );
                bug( buf, 0 );
                return;
            }
            ...
            {
                do_printf( buf, "Mobprog: invalid if_check (and), mob %u prog %u",
                        mvnum, pvnum );
                bug( buf, 0 );
                return;
            }
        ...
        else if ( !str_cmp( control, "endif" ) )
        {
            if ( !level || state[level-1] != BEGIN_BLOCK )
            {
                do_printf( buf, "Mobprog: endif without if, mob %u prog %u",
                        mvnum, pvnum );
                bug( buf, 0 );
                return;
            }
        ...
        else if ( !str_cmp( control, "else" ) )
        {
            if ( !level || state[level-1] != BEGIN_BLOCK )
            {
                do_printf( buf, "Mobprog: else without if, mob %u prog %u",
                        mvnum, pvnum );
                bug( buf, 0 );
                return;
            }
...
void mp_bribe_trigger( CHAR_DATA *mob, CHAR_DATA *ch, int64 amount )
{
  MPROG_LIST *prg;
  ...
  {    
       ...
       (HAS_TRIGGER(mob, TRIG_EXIT) || HAS_TRIGGER(mob, TRIG_EXALL)))
    {
      for ( prg = mob->pIndexData->mprogs; prg; prg = prg->next )
      {
        ...
        if ( prg->trig_type == TRIG_EXIT &&  dir == atoi( prg->trig_phrase )
         &&  mob->position == mob->pIndexData->default_pos && can_see( mob, ch,CHECK_LVL ) )
        {
          program_flow( prg->vnum, prg->code, mob, ch, NULL, NULL );
          return TRUE;
        }
